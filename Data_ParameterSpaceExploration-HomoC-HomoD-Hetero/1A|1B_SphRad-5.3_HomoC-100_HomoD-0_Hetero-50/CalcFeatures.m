function[features_vert,features_horz] = CalcFeatures(curDir,pars)
delete('*.png')
files = dir('*.txt');
all_conditions = [];
features_vert = [];
features_horz = [];
head = ["ratio",pars',"Feature","Value"];
...
condition_param_values = [ratio];
for j = 1:length(pars)-1
    condition_param_values = [condition_param_values (curDir(index1(j+1):index2(j+2)) + "")];
end
condition_param_values = [condition_param_values (curDir(index1(length(pars)+1):end) + "")];
filecount = 0;
for k = 1:length(files)
    ...
    for j = 1:length(A)/65
        ...
        file_path = curDir + "/" + img_name;
        row_features = [file_path condition_param_values];
    ...
    %         red_rad = mean((sqrt(points(:,1).^2 + points(:,2).^2)+sqrt(points(3).^2 + points(4).^2))/2);
        ...
        offset = offset + 65;
        row_features = [row_features col_features'];
        features_vert = vertcat(features_vert, col_features);
        features_horz = vertcat(features_horz,row_features);
        all_conditions = [all_conditions; repmat(condition_param_values, length(feature_name), 1)];
        feature_names=[feature_names;feature_name];
    end
end
    features_vert = [all_conditions,feature_names, features_vert];
    writematrix([head;features_vert], "features_" + curDir + ".xlsx");
end