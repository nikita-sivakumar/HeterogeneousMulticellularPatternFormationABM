...
globals[
  ...
  percent-saturation
  a-count b-count
  cluster-pause-delay cluster-move-size
...
;initialize global variables that are not affected by the parameter tuning
  set spheroid-radius 5.3
  set a-ratio 1
  set b-ratio 1
  set percent-saturation 0.65
  set cluster-pause-delay 3
  set cluster-move-size 5
  set heterotypic-prob 5
;  set homotypic-prob-c 90 set homotypic-prob-d 90
;  set heterotypic-prob 10
...
to adjust-exp
;  set exp-expression-const-c exp-expression-const-c / 100
;  set exp-expression-const-d exp-expression-const-d / 100
  set exp-expression-const exp-expression-const / 100
end
...
to express-c
  ...
  if n < (1 - exp (-1 * exp-expression-const * c / 8)) * 100[
    set color green
    set g-color true
  ]
;  if(c = 1)[
...
to setOutputFile
  ...
  set output_folder (word "Output_2.6/SensitivityAnalysis/SpheroidRadius_2/"a-ratio"A|"b-ratio"B_SphRad-"
    original-spheroid-radius"/")

;  set output_folder (word "Cluster-Asymm-Validation_3.1/"a-ratio"A|"b-ratio"B_SphRad-"original-spheroid-radius"/")
...
;
  while [file-exists? (word output_folder""date"_radius_"original-spheroid-radius""a-ratio"A|"b-ratio"_run_"fileCounter".png")][set fileCounter fileCounter + 1]
  let outputImageName (word output_folder""date"_radius_"original-spheroid-radius""a-ratio"A|"b-ratio"_run_"fileCounter".png")
...
to outputContigArea
  ...
  let contig-green count turtles with [color = orange]
  ...
  let contig-red count turtles with [color = white]

  file-write (word "contig-red")
  file-write contig-red
  file-print ""

  let contig-ratio contig-red / contig-green
  file-write (word "contig-ratio")
...
outputImages</final>
  ...
  <experiment name="1A:1B-HomoHetero" repetitions="30" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>makeOutputFile</final>
    <timeLimit steps="100"/>
